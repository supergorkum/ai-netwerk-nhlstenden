// Netlify Function (background) — AI-Koers: bouwt het rijke rapport.
//
// Dit is een ACHTERGRONDTAAK (config.background = true): de aanroeper
// krijgt direct een lege 202-response, en deze functie mag daarna tot
// 15 minuten doorlopen (Netlify Pro). Dat is nodig omdat Claude alle 16
// pagina's in één keer analyseert, wat te lang duurt voor de harde
// tijdslimiet van een gewone, synchrone function (vandaar de eerdere
// 502-fout).
//
// Het resultaat wordt weggeschreven naar Blobs onder de key
// 'ai-koers-rapport-status'. De snelle, gewone function
// ai-koers-rapport.js start deze achtergrondtaak en geeft de aanroeper
// een status terug; de pagina peilt daarna elke paar seconden of het
// resultaat al klaar staat.
//
...
export default async (req) => {
  ...
  const schrijfStatus = (obj) => store.set(STATUS_KEY, JSON.stringify(obj))

  try {
    const apiKey = process.env.ANTHROPIC_API_KEY
    if (!apiKey) {
      await schrijfStatus({ status: 'fout', foutOp: new Date().toISOString(), foutmelding: 'Geen Anthropic API key ingesteld.' })
      return
    }
    ...
    try {
      ...
      await schrijfStatus({ status: 'fout', foutOp: new Date().toISOString(), foutmelding: `Kon paginas.json niet laden: ${err.message}` })
      return
    }
...
De paginas-array moet precies ${aantalPaginas} items hebben, index 0 tot en met ${aantalPaginas - 1}, in volgorde.`
    ...
    try {
      ...
      await schrijfStatus({ status: 'fout', foutOp: new Date().toISOString(), foutmelding: `Analyse mislukt: ${err.message}` })
      return
    }
    ...
    await schrijfStatus({ status: 'klaar', klaarOp: new Date().toISOString(), rapport })
  } catch (err) {
    // Vangnet: welke fout er ook onverwacht optreedt, de status mag nooit
    // voor altijd op 'bezig' blijven staan, anders zit de pagina vast.
    try {
      await schrijfStatus({ status: 'fout', foutOp: new Date().toISOString(), foutmelding: `Onverwachte fout: ${err.message}` })
    } catch { /* zelfs dit opslaan kan mislukken, dan is er niets meer aan te doen */ }
  }
}

export const config = { path: '/.netlify/functions/ai-koers-rapport-background', background: true }
